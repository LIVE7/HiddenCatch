...
public class GameRank extends JFrame {
	...
	public void updateRank() {
		...
		//맞춘 개수가 더 많으면 값을 update
		if(iNumOfCorAns < PanelController.gameMain.numOfCorAns) {
			iNumOfCorAns = PanelController.gameMain.numOfCorAns;
			iClearTime = PanelController.gameMain.myTimer.getCurrnetTime();
			
			lbEasy.setText("I D  :  "+ strId);
			lbNormal.setText("맞춘 문제  :  "+ iNumOfCorAns +" 개");
			lbHard.setText("남은 시간  :  "+ iClearTime + " 초");
		
		//맞춘 개수가 같다면 남은시간이 더 많은 값을 update
		} else if (iNumOfCorAns == PanelController.gameMain.numOfCorAns) {
			if(iClearTime < PanelController.gameMain.myTimer.getCurrnetTime()) {
				iNumOfCorAns = PanelController.gameMain.numOfCorAns;
				iClearTime = PanelController.gameMain.myTimer.getCurrnetTime();
